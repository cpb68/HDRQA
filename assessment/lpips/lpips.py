...
def spatial_average_w(in_tens,w, keepdim=True):
    error = in_tens.mean([2,3],keepdim=keepdim)
    weight = w.mean([2,3],keepdim=keepdim)
    return error/weight
...
class LPIPS(nn.Module):
    ...
    def forward(self, in0, in1,w, retPerLayer=False, normalize=False):
        ...
        feats0, feats1, diffs,weight = {}, {}, {},{}

        for kk in range(self.L):
            feats0[kk], feats1[kk] = lpips.normalize_tensor(outs0[kk]), lpips.normalize_tensor(outs1[kk])
            b, c, ww, hh = feats0[kk].shape
            weight[kk] = F.interpolate(w, size=(ww, hh), mode='nearest')
            diffs[kk] = weight[kk]*(feats0[kk]-feats1[kk])**2

        if(self.lpips):
            if(self.spatial):
                res = [upsample(self.lins[kk](diffs[kk]), out_HW=in0.shape[2:]) for kk in range(self.L)]
            else:
                res = [spatial_average_w(self.lins[kk](diffs[kk]),weight[kk], keepdim=True) for kk in range(self.L)]
        else:
            if(self.spatial):
